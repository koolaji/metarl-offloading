        ...
        for i in range(self.meta_batch_size):
            ...
            self.assign_old_eq_new_tasks.append(
                U.function([], [], updates=[tf.compat.v1.assign(oldv, newv)
                                            for (oldv, newv) in
                                            zipsame(self.core_policy.get_variables(),self.meta_policies[i].get_variables())])
            )
...
class MTLBO:
    def __init__(self, population_size, dim, bounds, iterations, trainer, start_W=0.9, end_W=0.2):
        def generate_bounded_random():
                new_solution = np.empty(self.dim)
                for dim in range(self.dim):
                    lower, upper = self.bounds[dim]
                    if lower == int(lower):  # Check underlying type
                        new_solution[dim] = np.random.randint(self.bounds[dim, 0], self.bounds[dim, 1] + 1)
                    else:
                        new_solution[dim] = self.bounds[dim, 0] + np.random.rand() * (self.bounds[dim, 1] - self.bounds[dim, 0])
                return new_solution           
        ...
        self.population = np.array([generate_bounded_random() for _ in range(population_size)])
        self.fitness = np.zeros(population_size)
        self.teacher = 10000000
    ...
    def teacher_phase(self):
        ...
        # Helper function to generate bounded random values (updated)
        def generate_bounded_random(bounds):
            new_solution = np.empty(self.dim)
            for dim in range(self.dim):
                lower, upper = bounds[dim]
                if lower == int(lower):  # Check underlying type
                    new_solution[dim] = np.random.randint(bounds[dim, 0], bounds[dim, 1] + 1)
                else:
                    new_solution[dim] = bounds[dim, 0] + np.random.rand() * (bounds[dim, 1] - bounds[dim, 0])
            return new_solution

        new_population = []
        for i in range(self.population_size):
            tf = np.random.randint(1, 3)
            diff = np.abs(generate_bounded_random(self.bounds) - tf * mean_solution)
            print(diff)
            W = self.start_W - (self.start_W - self.end_W) * (self.current_iteration / self.max_iterations)

            if self.fitness[i] < mean_fitness:  # Superior student
                new_solution = np.abs(self.population[i] * W + (best_solution - self.population[i]) * np.random.rand())
            else:  # Underachieving student
                new_solution = np.abs((self.population[i] + (np.random.rand(self.dim) - 0.5) * 2 * (mean_solution - self.population[i])) * np.sin(
                    np.pi / 2 * self.current_iteration / self.max_iterations
                ) + diff * np.cos(np.pi / 2 * self.current_iteration / self.max_iterations))
            print(new_solution)
        ...
        self.evaluate_population(sess=sess)  # Evaluate fitness for the new population
    ...
    def learner_phase(self):
        ...
        new_population = []
        for i in range(self.population_size):
            if i < midpoint:  # Superior student
                neighbor = np.random.choice(sorted_indices[:midpoint])
                if self.fitness[i] > self.fitness[neighbor]:
                    new_solution = self.population[i] + (self.population[neighbor] - self.population[i]) * np.cos(
                        np.pi / 2 * self.current_iteration / self.max_iterations
                    )
                else:
                    new_solution = self.population[i] + (generate_bounded_random(self.bounds) - 0.5) * 2 * (self.bounds[:, 1] - self.bounds[:, 0])
            else:  # Underachieving student
                best_index = np.argmin(self.fitness)
                new_solution = self.population[i] + (self.population[best_index] - self.population[i]) * np.cos(
                    np.pi / 2 * self.current_iteration / self.max_iterations
                )

            # Enforce bounds without np.clip
            for dim in range(self.dim):
                lower, upper = self.bounds[dim]
                new_solution[dim] = max(lower, min(new_solution[dim], upper))
        ...
        for i in range(self.population_size):
            if tmp_fitness[i] < self.fitness[i]:
                self.fitness[i] = tmp_fitness[i]
                self.population[i] = tmp_population[i]  # Corrected index
