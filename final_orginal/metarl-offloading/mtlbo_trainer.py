...
class TLBO:
    ...
    def teacher_phase(self):
        ...
        new_solution_list = [] * self.population_size
        for i in range(self.population_size):
            tf = np.random.randint(1, 3)
            diff = np.random.rand(self.dim) * (best_solution - tf * mean_solution)
            ...
            if self.fitness[i] < mean_fitness:  # Superior student
                new_solution = self.population[i] * W + (best_solution - self.population[i]) * np.random.rand()
            else:  # Underachieving student
                new_solution = (self.population[i] + (np.random.rand(self.dim) - 0.5) * 2 * (mean_solution - self.population[i])) * np.sin(
        ...
        self.evaluate_population(sess=sess)  # Assuming 'sess' is defined elsewhere
        for i in range(self.population_size):
        ...
        for i in range(self.population_size):
            if i < midpoint:  # Superior student
                ...
                if self.fitness[i] > self.fitness[neighbor]:
                    ...
                    new_solution = self.population[i] + (np.random.rand(self.dim) - 0.5) * 2 * (self.bounds[:, 1] - self.bounds[:, 0])
            else:  # Underachieving student
                best_index = np.argmin(self.fitness)
    ...
    def optimize(self, sess):
        ...
        for _ in range(self.iterations):
            ...
            self.trainer.policy.core_policy.save_variables(save_path=f"./meta_model_inner_step1/meta_model_{str(_)}.ckpt")
