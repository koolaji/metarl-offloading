...
class MetaIterativeEnvExecutor(object):
    def __init__(self, env, meta_batch_size, envs_per_task, max_path_length):
        logging.debug('Start MetaIterativeEnvExecutor')
    ...
    def step(self, actions):
        logging.debug('Start MetaIterativeEnvExecutor step')
        assert len(actions) == self.num_envs
        all_results = [env.step(a) for (a, env) in zip(actions, self.envs)]
        # logging.debug('all_results = %s', all_results)
    ...
    def set_tasks(self, tasks):
        logging.debug('Start MetaIterativeEnvExecutor set_tasks')
