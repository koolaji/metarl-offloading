        ...
        for i in range(num_learners):
            ...
            if i != partner_index:
                ...
                for key in learners[i]:
                    if random.random() < 0.5:
                        if objective_func(learners[i]) < objective_func(partner):
                            learners[i][key] += random.uniform(0.1, 0.5) * (partner[key] - learners[i][key])
                        else:
                            learners[i][key] -= random.uniform(0.1, 0.5) * (partner[key] - learners[i][key])

        # Evaluate and update the best parameterss
        for learner in learners:
            score = objective_func(learner)
            if score < best_score:
                best_score = score
                best_params = learner.copy()
                print(f"Best MSE so far: {best_score:.15f}")
        
        print(f"Generation {generation+1}: Best MSE so far: {best_score:.15f}")
