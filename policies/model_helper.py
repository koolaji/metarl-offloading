...
def _single_cell(unit_type, num_units, forget_bias, dropout, mode,
                 residual_connection=False, device_str=None, residual_fn=None, attention_mechanism=None):
    """Create an instance of a single RNN cell."""
    # dropout (= 1 - keep_prob) is set to 0 during eval and infer
    dropout = dropout if mode == tf.contrib.learn.ModeKeys.TRAIN else 0.0

    # Cell Type
    logging.info(f"unit_type = {unit_type}")
    if unit_type == "lstm":
        single_cell = tf.contrib.rnn.BasicLSTMCell(num_units, forget_bias=forget_bias)
    elif unit_type == "peephole_lstm":
        single_cell = tf.contrib.rnn.LSTMCell(num_units, use_peepholes=True, forget_bias=forget_bias)
    elif unit_type == "gru":
    ...
    if attention_mechanism is not None:
        single_cell = tf.contrib.seq2seq.AttentionWrapper(single_cell, attention_mechanism, attention_layer_size=num_units)
