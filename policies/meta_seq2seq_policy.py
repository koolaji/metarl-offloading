...
class Seq2SeqNetwork():
    def __init__(self, name,
        ...
        with tf.compat.v1.variable_scope(name, reuse=self.reuse, initializer=tf.glorot_normal_initializer()):
            ...
            """
            In summary, this code defines a sequence-to-sequence model with three decoders: one for training, one for sampling, and one for greedy decoding.
            The model learns to map input sequences to output sequences, and can generate new sequences in different ways depending on the decoder used. 
            The pi, q, and vf values are used for reinforcement learning. The pi represents the policy (probability distribution over actions), 
            the q represents the Q-values (expected return of taking an action in a given state), and the vf represents the value function (expected return of a state). 
            The model is trained to maximize the expected return. The embeddings are learned representations of the inputs, and the decoders generate outputs 
            based on these embeddings and the encoder’s state.
            """
    def predict_training(self, sess, encoder_input_batch, decoder_input, decoder_full_length):
        """
        In summary, this method is used to make predictions with the trained sequence-to-sequence model. It feeds the encoder and decoder inputs 
        to the model and returns the model’s predictions and policy. The policy (self.pi) is a probability distribution over actions, representing 
        the model’s strategy for selecting actions. The predictions (self.decoder_prediction) are the actions selected by the model according to this policy. 
        This method is likely used during the training process to evaluate the model’s performance on the training data.
        """
